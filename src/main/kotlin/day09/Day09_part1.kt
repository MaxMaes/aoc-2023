...
fun main() {
    ...
    val extrapolated = zeroed.map { tree ->
        ...
        for (i in tree.lastIndex downTo 1) {
            ...
            // Take the last value from the last line
            val lastValue = lastLine.last()
            // Take the last value from the next line
            val nextValue = nextLine.last()
    ...
    println(extrapolated.map { it.first() }.sumOf { it.last() })
}